...
T& CommonHelper::GetValue(std::vector<T>& val_list, std::vector<int32_t> shape, std::vector<int32_t> pos)
{
    ...
    for (int32_t i = 0; i < stride.size(); i++) {
...
{
    int32_t element_num = std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int32_t>());
    if (val_list.size() != element_num) {
        PRINT_E("[PrintValue] invalid data size. %zd, %d\n", val_list.size(), element_num);
        return;
    }
    ...
    for (int32_t i = 0; i < element_num; i++) {
        ...
        for (int32_t j = 0; j < stride.size(); j++) {
